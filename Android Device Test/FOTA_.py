...
class FOTA():
    ...
    def goToFotaMenu(self):
        ...
        if CALL helpers_sea.device_config.walkThroughMenuTree("Settings", ["(?i)Software update.*",
            ...
            CALL clickAnObject(text="Continue", type=Types.Button, critical=False)

            CALL waitForObjectToDisappear(type=Types.ProgressBar, timeout=Delays.NetworkReaction)
            if CALL waitForObjectToAppear(text="(?i).*(up to date|Download updates|Update your|ready to install).*"):
                return True

            if CALL waitForObjectToAppear(text="(?i)(Download|Install.*)", type=Types.Button):
                return True
    ...
    def connectWiFiForFota(self):
        ...
        for _ in xrange(2):
            if not CALL checkIfNetworkTypeIsActive("WiFi"):
                if CALL helpers_sea.wifi.connectWiFi(CALL stpPropertiesFileManager.getProperty("WifiName"),
                                                         CALL stpPropertiesFileManager.getProperty(
                                                             "WifiPasswd")):
                    return True
                CALL goHomeScreenByPressingBackKeys()
    ...
    def checkFotaStatus(self, wifi_only=False, install_dummy=True):
        ...
        if CALL waitForObjectToAppear(text="(?i).*Failed to update.*"):
            CALL clickAnObject(text="(?i)OK", type=Types.Button, critical=False)
            self.goToFotaMenu()
        ...
        if CALL waitForObjectToAppear(text="(?i)Download", type=Types.Button):
        ...
        if CALL waitForObjectToAppear(text="(?i)Install Now", type=Types.Button):
    ...
    def getSwVersionForNextUpdate(self):
        ...
        """
        sw_ver = ""
        if CALL slideUpDownToFindObject(type=Types.TextView, text="Software update information",
    ...
    def getCurrentSwDetailFromDevice(self, reset=False, isDummy=False):
        ...
        for _ in xrange(2):
            try:
                CALL helpers.calls.goPhoneAndInputNumber("*#06#", viaADB=True)
                self.takeFotaScreenshot("DBGSCR_IMEI_SV")
                break
            except:
                CALL writeLog("!! Error occurred (*#06#): Trying again..")
            finally:
        ...
        for _ in xrange(2):
            try:
                CALL helpers.calls.goPhoneAndInputNumber("*#12580*369#", viaADB=True)
                self.takeFotaScreenshot("DBGSCR_SWVER")
                break
            except:
                CALL writeLog("!! Error occurred (*#12580*369#): Trying again..")
            finally:
                CALL goHomeScreenByPressingBackKeys()
    ...
    def downloadUpdate(self):
        ...
        if not CALL waitForObjectToAppear(text="(?i)Download", type=Types.Button):
        ...
        for _ in range(2):  # try download one more time, in case first attempt is failed
            ...
            if not CALL clickAnObject(text="(?i)Download", type=Types.Button, critical=False):
                continue
    ...
    def installUpdate(self):
        ...
        CALL clickAnObject(type=Types.Button, text="(?i).*Install now.*")
        ...
        critical = "PIN" in CALL cmdManager.getProp("gsm.sim.state",
        ...
        while "ERROR" not in res and res != FotaState.UPDATE_NOT_FOUND:
            ...
            while res == FotaState.DOWNLOADING:
                ...
                while remaining >= 0:
                    ...
                    CALL clickAnObject(text="Continue", type=Types.Button, critical=False, timeout=Delays.Wait2)
    ...
    def getFotaFileName(self, file_name, with_swver=False):
        ...
        """
        sw_info = ""
        model = str(CALL cmdManager.getProp(propName="ro.product.model")).split("-")[1]
        if with_swver:
            apver = str(CALL cmdManager.getProp(propName="ro.build.PDA"))[-4:]
            cpver = str(CALL cmdManager.getProp(propName="ril.sw_ver"))[-4:]
            cscver = str(CALL cmdManager.getProp(propName="ril.official_cscver"))[-4:]  # or ro.omc.build.version
